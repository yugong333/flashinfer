...
##### MonoMoe (single-kernel block-FP8 top-K MoE, SM90a)

The MoE monokernel (`flashinfer/fused_moe/monomoe.py`, kernel in
`csrc/fused_moe/monomoe/`) serves a **curated set of shapes** declared in
`csrc/fused_moe/monomoe/shapes.json` (keyed by `(E, N, K)`).  Each registered
shape JIT-compiles into its own module, baking ALL of that shape's tunable
`KernelConfig` variants (grid / down-col-tile / K-step-up/down / weight-TMA
slots / down-pipe-depth); a runtime `config_id` selects among them with no
rebuild.  An unregistered `(E, N, K)` is refused — add it to `shapes.json`,
regenerate (`tools/gen_shapes.py`), and rebuild.

`config_id` resolution (highest precedence first): the explicit `config_id=`
kwarg on `mono_moe` → the `MONOMOE_CONFIG` env var → the tuner-emitted JSON
(`MONOMOE_TUNED_JSON`, else the default dir) → config 0 (the shipped default,
byte-identical to the pre-tuning kernel).  **Both the `MONOMOE_CONFIG` per-M
syntax and the tuned JSON select the config from the runtime batch size M**
(largest pinned/tuned M′ ≤ M, matching the BS8/BS16 bucketing).

```bash
MONOMOE_CONFIG=2                                  # config id 2 for any shape / M
MONOMOE_CONFIG=e256_n512_k2048:1                  # per-shape, all M (shape key)
MONOMOE_CONFIG=E256N512K2048:1                    # per-shape, all M (E/N/K key)
MONOMOE_CONFIG=e256_n512_k2048@1:3,e256_n512_k2048@16:7  # per (shape, M): cfg3 for M>=1, cfg7 for M>=16
MONOMOE_CONFIG=E256N512K2048@8:6                  # per (shape, M) by E/N/K key
MONOMOE_TUNED_JSON=best_e256_n512_k2048.json      # explicit tuned JSON (per-M) path
MONOMOE_REQUIRE_TUNED=1                           # error (not warn+config0) if untuned
```

With no env var, `mono_moe` reads the tuner output from the **default dir**
`$FLASHINFER_WORKSPACE_DIR/monomoe_tuned/<key>.json` (where `tune_monomoe.py`
writes when `--json` is omitted), so a tuned deployment is per-M-selected
automatically.

| Variable | Default | Read in | Effect |
|----------|---------|---------|--------|
| `MONOMOE_CONFIG` | unset | `flashinfer/fused_moe/monomoe.py` | Pins a `config_id`. Forms: bare `id`; per-shape `shape:id` (all M); per-(shape, M) `shape@M:id` (applies to token counts ≥ M — largest pinned M′ ≤ M wins). `shape` is the key/alias or the `E{E}N{N}K{K}` key. Overridden by an explicit `config_id=` kwarg. `MONOKERNEL_CONFIG` is a deprecated alias. |
| `MONOMOE_TUNED_JSON` | unset | `flashinfer/fused_moe/monomoe.py` | Explicit path to the `tools/tune_monomoe.py` best-config-per-M JSON. Consulted when no kwarg/env pins a config; picks the largest tuned M ≤ the token count. When unset, the default dir `$FLASHINFER_WORKSPACE_DIR/monomoe_tuned/` is searched (`<key>.json` then `monomoe_tuned.json`). |
| `MONOMOE_REQUIRE_TUNED` | `0` | `flashinfer/fused_moe/monomoe.py` | `1` raises for a registered-but-untuned (shape, M) instead of warning once and falling back to config 0. |

Config tables are generated from `shapes.json` into
`csrc/fused_moe/monomoe/generated/configs_generated.inc` (the `MONO_CONFIGS_*`
X-macros); `tools/enum_configs.py` enumerates the feasible candidate set for a
new shape; `tools/tune_monomoe.py` (Ray multi-GPU, in-process fallback) picks
the best config per (shape, M) and writes to the default dir (or `--json`).
See `docs/design_docs/monomoe_kernel.md`.
