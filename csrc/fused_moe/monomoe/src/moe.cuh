 ...
 * docs/design_docs/monomoe_kernel.md for the architecture and
 * moe_interface.h for the entry-point documentation.
 */
...
/**
 * @brief Top-K MoE kernel — split-phase TMA+WGMMA path for BS <= 16.
 *
 * Pipeline (see docs/design_docs/monomoe_kernel.md "Phase pipeline"):
 *   Phase 1: routing (topK) ∥ routing-window TMA (full BF16 input tile
 *            into bf16_in_full, completion on bar_rwin)
 *   Phase 2: warp 0 runs prepare_moe_topk; warps 1..11 wait on
...
__device__ void moe_kernel_topk_impl(
  ...
  static_assert(Dims::BS <= 16,
                "moe_kernel_topk_impl supports BS<=16 (the BS16 tag reuses "
...
#pragma unroll
  ...
  if (in_up && up_group < shmem->expert_count) {
    ...
    // layout, UP_COL_HALVES == 2) vs the single-atom interleaved layout
    // (UP_COL_HALVES == 1).
    if constexpr (CoreDims::UP_COL_HALVES == 2u) {
      moe_up_projection_two_atom_wgmma_tma<Dims>(activations_in, expert_weights_up,
