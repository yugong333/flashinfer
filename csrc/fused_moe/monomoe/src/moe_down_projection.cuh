...
/**
 * @brief TMA + WGMMA down-projection for BS <= 16.
 *
...
__device__ inline void moe_down_projection_allexperts_wgmma_tma(
  ...
  static_assert(Dims::BS <= 16,
                "moe_down_projection_allexperts_wgmma_tma supports "
...
#pragma unroll
    ...
    if constexpr (Dims::BS == 16) {
      if (!need_first_expert_prime && is_tma_launcher_thread<Dims>() && routed_count > 0u) {
        // Mirror the prime: tiles 0..PRIME_STEPS-1 (1 for the 2-deep
        // buffer, 2 for the 4-deep ring); later tiles come from the
        // in-loop s+1 / (s+2)&3 arms as usual.  Slots 0,1 are safe to
        // fill here: their last consumers were the previous expert's
        // K-steps, ordered behind the expert-bottom sync.
        constexpr std::uint32_t ACT_TOP_STEPS = (DOWN_PIPE_DEPTH == 4u) ? 2u : 1u;
#pragma unroll
...
#pragma unroll
            for (std::uint32_t j = 0; j < 2; ++j) {
              ...
              if constexpr (Dims::BS == 16) {
                ...
                // chunk_d_lo_hi.
                wgmma_m64n16k32_e4m3_e4m3_f32(desc_a, desc_b, chunk_d_lo[h][0], chunk_d_lo[h][1],
                                              chunk_d_lo[h][2], chunk_d_lo[h][3],
                                              chunk_d_lo_hi[h][0], chunk_d_lo_hi[h][1],
                                              chunk_d_lo_hi[h][2], chunk_d_lo_hi[h][3]);
              } else {
...
#pragma unroll
            for (std::uint32_t j = 2; j < WGMMAS_PER_SUBSTEP; ++j) {
              ...
              std::uint64_t desc_b = make_wgmma_desc(b_ptr, B_LBO, B_SBO, B_SWIZZLE);
              if constexpr (Dims::BS == 16) {
                wgmma_m64n16k32_e4m3_e4m3_f32(desc_a, desc_b, chunk_d_hi[h][0], chunk_d_hi[h][1],
                                              chunk_d_hi[h][2], chunk_d_hi[h][3],
                                              chunk_d_hi_hi[h][0], chunk_d_hi_hi[h][1],
                                              chunk_d_hi_hi[h][2], chunk_d_hi_hi[h][3]);
              } else {
...
#pragma unroll
            for (std::uint32_t h = 0; h < DOWN_COL_HALVES; ++h) {
              const float ws = shm->w_down_scale[h][my_wg][ws_col];
              final_d_hi[h][0] +=
                  chunk_d_lo_hi[h][0] * as_lo_46 * ws + chunk_d_hi_hi[h][0] * as_hi_46 * ws;
              final_d_hi[h][1] +=
                  chunk_d_lo_hi[h][1] * as_lo_57 * ws + chunk_d_hi_hi[h][1] * as_hi_57 * ws;
              final_d_hi[h][2] +=
                  chunk_d_lo_hi[h][2] * as_lo_46 * ws + chunk_d_hi_hi[h][2] * as_hi_46 * ws;
              final_d_hi[h][3] +=
                  chunk_d_lo_hi[h][3] * as_lo_57 * ws + chunk_d_hi_hi[h][3] * as_hi_57 * ws;
      ...
      if (is_prefetch_warp<Dims>()) {
        ...
        if (has_prev_expert && s < NUM_ACC_STEPS) {
          ...
          if constexpr (Dims::BS == 16) {
            ...
            for (unsigned rc = start + thread_in_pf; rc < end; rc += PF_THREADS) {
...
#pragma unroll
      for (std::uint32_t h = 0; h < DOWN_COL_HALVES; ++h) {
        ...
        if constexpr (Dims::BS == 16) {
          ...
          shm->partial_result.down_out[row_base + 0][col_base + 8] = final_d_hi[h][0];
          shm->partial_result.down_out[row_base + 0][col_base + 9] = final_d_hi[h][1];
          shm->partial_result.down_out[row_base + 8][col_base + 8] = final_d_hi[h][2];
          shm->partial_result.down_out[row_base + 8][col_base + 9] = final_d_hi[h][3];
        }
      }
    }
  ...
  if (expert_count > down_group) {
    ...
    if constexpr (Dims::BS == 16) {
      ...
      // K-loop is done.
      const std::uint32_t last_start = static_cast<std::uint32_t>(shm->expert_slot_start[last_id]);
      const std::uint32_t last_rc = static_cast<std::uint32_t>(shm->expert_routed_count[last_id]);
      for (unsigned rc = thread_in_block; rc < last_rc * DOWN_COL_TILE; rc += blockDim.x) {
        const unsigned rank = rc / DOWN_COL_TILE;
        const unsigned col = rc % DOWN_COL_TILE;
