...
/**
 * @brief Up-projection for BS <= 16, single-atom interleaved layout
 * (UP_COL_HALVES == 1).
 *
...
__device__ inline void moe_up_projection_allexperts_wgmma_tma(
  ...
  constexpr uint64_t B_SBO =
      (Dims::BS == 16u)
          ? (8ull * static_cast<uint64_t>(MoE_SHM<Dims>::TinyDataWGMMA_TMA::FP8_ACT_K_CHUNK))
          : B_LBO;
...
#pragma unroll
          for (uint32_t j = 0; j < WGMMAS_PER_SUBSTEP; ++j) {
            ...
            if constexpr (Dims::BS == 16) {
              ...
              wgmma_m64n16k32_e4m3_e4m3_f32(desc_a, desc_b, chunk_d0, chunk_d1, chunk_d2, chunk_d3,
    ...
    // stores are deferred to the PF body above during the next expert.
    {
      if (is_calc) {
        ...
        if constexpr (Dims::BS == 16) {
          ...
          float val2 = __fdividef(rw_46 * final_d6 * final_d4, 1.0f + __expf(-final_d4));
          float val3 = __fdividef(rw_57 * final_d7 * final_d5, 1.0f + __expf(-final_d5));
...
/**
 * @brief Up-projection for BS <= 16, two-atom raw layout
 * (UP_COL_HALVES == 2, incl. the decoupled up/down-grid shapes).
 *
 ...
 */
template <typename Dims>
__device__ inline void moe_up_projection_two_atom_wgmma_tma(
    const A_element* __restrict__ activations_in, const W_element* __restrict__ expert_weights_up,
    const S_element* __restrict__ expert_scales_up, std::uint32_t top_k, std::uint32_t batch_size,
    MoEGemmSpec<Dims>* __restrict__ spec, MoE_SHM<Dims>* __restrict__ shmem,
    CUtensorMap const& up_weights_desc, CUtensorMap const& activations_desc,
  ...
  static_assert(CoreDims::UP_COL_HALVES == 2u,
                "moe_up_projection_two_atom_wgmma_tma is specialized for "
                "UP_COL_HALVES == 2.");
...
#pragma unroll
  for (uint32_t h = 0; h < HALVES; ++h) {
    ...
    final_d_hi[h][0] = final_d_hi[h][1] = final_d_hi[h][2] = final_d_hi[h][3] = 0.f;
  }

  constexpr uint32_t UP_W_TX_BYTES_PER_ATOM = 16384u;
  constexpr uint32_t UP_W_TX_BYTES_TOTAL = UP_W_TX_BYTES_PER_ATOM * K_SUBSTEPS * HALVES;
...
#pragma unroll
      for (uint32_t h = 0; h < HALVES; ++h) {
        ...
        tma_load_up_wgmma_tile_raw(up_weights_desc, /*expert_id=*/eid, /*N=*/Dims::N,
...
#pragma unroll
          for (uint32_t h = 0; h < HALVES; ++h) {
            ...
            const uint32_t atom = kk * HALVES + h;
            const void* a_kk_base =
                (const void*)((const char*)a_slot_base + atom * ATOM_W_BYTES + wg_offset_bytes);
            chunk_d0 = chunk_d1 = chunk_d2 = chunk_d3 = 0.f;
            if constexpr (Dims::BS == 16) {
              chunk_d4 = chunk_d5 = chunk_d6 = chunk_d7 = 0.f;
            }
#pragma unroll
            for (uint32_t j = 0; j < WGMMAS_PER_SUBSTEP; ++j) {
              ...
              uint64_t desc_b = make_wgmma_desc(b_ptr, B_LBO, B_SBO, 0);
              if constexpr (Dims::BS == 16) {
                wgmma_m64n16k32_e4m3_e4m3_f32(desc_a, desc_b, chunk_d0, chunk_d1, chunk_d2,
                                              chunk_d3, chunk_d4, chunk_d5, chunk_d6, chunk_d7);
              } else {
                wgmma_m64n8k32_e4m3_e4m3_f32(desc_a, desc_b, chunk_d0, chunk_d1, chunk_d2,
                                             chunk_d3);
              }
            }
            wgmma_commit_group();
            ...
            const float as_13 = shmem->act_scale[kblk][tok_13];
            const S_element* ws_buf = shm->up_scale[cur_scale_slot];
            const float ws = (h == 0u) ? ws_buf[kblk + 0u] : ws_buf[kblk + UP_SCALE_COLS];
...
#pragma unroll
          ...
          float local_max = fmaxf(fmaxf(fabsf(v[0]), fabsf(v[1])), fmaxf(fabsf(v[2]), fabsf(v[3])));
...
#pragma unroll
            for (int i = 0; i < 4; ++i) {
              if (wr[i]) {
                spec->temp_fp8[dest_row_local * Dims::N + oc[i]] = (AQ_element)(v[i] * inv_scale);
              }
            }
            // Sentinel handoff publish (see moe_publish_act_scale).
            moe_publish_act_scale<Dims>(spec, shmem->scale_parity, dest_row_local, effective_bid,
    ...
    // ── Calc-warp epilogue: combine silu(gate)*up across the two atoms ──
    {
      if (is_calc) {
        ...
        float v_lo_even = __fdividef(rw_even * ud[0] * gd[0], 1.0f + __expf(-gd[0]));
        float v_lo_odd = __fdividef(rw_odd * ud[1] * gd[1], 1.0f + __expf(-gd[1]));
        float v_hi_even = __fdividef(rw_even * ud[2] * gd[2], 1.0f + __expf(-gd[2]));
        float v_hi_odd = __fdividef(rw_odd * ud[3] * gd[3], 1.0f + __expf(-gd[3]));
        if (!store_even) {
          v_lo_even = 0.f;
          v_hi_even = 0.f;
        }
        ...
        if constexpr (Dims::BS == 16) {
          ...
          float v_lo_even_hi =
              __fdividef(rw_even_hi * ud_hi[0] * gd_hi[0], 1.0f + __expf(-gd_hi[0]));
          float v_lo_odd_hi = __fdividef(rw_odd_hi * ud_hi[1] * gd_hi[1], 1.0f + __expf(-gd_hi[1]));
          float v_hi_even_hi =
              __fdividef(rw_even_hi * ud_hi[2] * gd_hi[2], 1.0f + __expf(-gd_hi[2]));
          float v_hi_odd_hi = __fdividef(rw_odd_hi * ud_hi[3] * gd_hi[3], 1.0f + __expf(-gd_hi[3]));
          if (!store_even_hi) {
            v_lo_even_hi = 0.f;
            v_hi_even_hi = 0.f;
          }
          ...
          shm->partial_result.post_silu_scratch[f_lo][tok_even_hi] = v_lo_even_hi;
          shm->partial_result.post_silu_scratch[f_lo][tok_odd_hi] = v_lo_odd_hi;
          shm->partial_result.post_silu_scratch[f_hi][tok_even_hi] = v_hi_even_hi;
        ...
        if (thread_in_block < Dims::BS) {
          shm->up_rank_for_tok_prev[thread_in_block] = shm->up_rank_for_tok[thread_in_block];
        }
      }
      has_pending_writeback = true;
    }
