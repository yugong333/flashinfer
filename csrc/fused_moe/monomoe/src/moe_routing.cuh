 ...
 * prepare_moe_topk's ranking would consume garbage ids.  The second
...
__device__ void topK(uint32_t top_k, ScoringFunc scoring_func, bool renormalize,
  ...
  if constexpr (Dims::BS == 16) {
    ...
    if (tok2 < num_tokens) {
      topK_one_token<Dims>(tok2, top_k, scoring_func, renormalize, router_logits, shmem,
                           expert_bias, routed_scaling_factor);
    }
  }
}
...
__device__ void prepare_moe_topk(uint32_t batch_size, uint32_t top_k,
  ...
  static_assert(Dims::BS <= 16, "prepare_moe_topk supports BS<=16");
