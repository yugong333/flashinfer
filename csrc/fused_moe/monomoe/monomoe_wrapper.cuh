...
// `TVM_FFI_ICHECK`.  Self-contained CUDA status check (no vLLM cuda_utils.h).
 ...
 * Every shape/config variant funnels through this single function;
 * monomoe_binding.cu instantiates it per config in the active shape's table.
 */
template <class Dims>
void monomoe_topk_launcher(TensorView activations_in, TensorView router_logits,
  ...
  // TMA descriptors (docs/design_docs/monomoe_kernel.md §5).  Non-TMA
  // variants leave these zero-initialized; only the TMA path reads them.
  // Per-tensor caller contracts live in the moe_tma.h factory docs.
  ...
  if constexpr (use_tma<Dims>::value) {
    ...
    activations_desc =
        create_activations_tma_desc(reinterpret_cast<const void*>(activations_in_ptr), num_tokens,
    ...
    // scratchpad: base + compile-time offset (design doc §4).
    ...
    down_activations_desc =
        create_down_activation_tma_desc(temp_fp8_ptr, MoEGemmSpec<Dims>::TEMP_ROWS_TMA, Dims::N,
                                        /*t_tile=*/MoECoreDims<Dims>::T_TILE);
  }
...
// No explicit instantiations here: monomoe_binding.cu #includes this header
// and instantiates monomoe_topk_launcher<Dims> implicitly, once per config in
// the active shape's generated table (DimsForConfig over MONO_ACTIVE_CONFIGS /
// _BS16).  config_id 0 uses the bare Base type, so its instantiation is
// byte-identical to the pre-tuning shipped kernel.
